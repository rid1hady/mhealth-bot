...
def construct_location_message(data):
    def get_open_now(open_now):
        ...
        if open_now:
            return TextComponent(text="Buka", size="sm", margin="xs", color="#1DB446")
        else:
            return TextComponent(text="Tutup", size="sm", margin="xs", color="#ff334b")
    ...
    bubble = BubbleContainer(
        ...
        body=BoxComponent(
            ...
            contents=[
                ...
                TextComponent(
                    text="⭐ {0} ({1} Ulasan)".format(
                        data.get("rating"), data.get("rating_user")
                    ),
                    size="sm",
                    margin="xs",
                    color="#aaaaaa",
...
def construct_mc_header(data):
    ...
    if question_number is None:
        return [
            TextComponent(
                text="Pilih salah satu", size="xs", margin="xs", color="#27ACB2"
            )
        ]
    else:
        ...
        actions.append(
            ButtonComponent(
                ...
                action=MessageAction(
                    label=button.get("title"), text=button.get("title")
                ),
            )
        )
